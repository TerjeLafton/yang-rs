...
impl ModuleLoader {
    ...
    pub fn load_file<P: AsRef<Path>>(mut self, path: P) -> Result<YangFile, ParserError> {
        ...
        // Collect imports from the parser
        let imports = parser.take_imports();

        // Process all imported modules and merge their reference nodes
        self.process_imports_with_list(path, &module.name, imports)?;

        // Create resolver with all reference information (local and imported)
        let resolver = ReferenceResolver::new(
            parser.get_reference_nodes().groupings.clone(),
            self.imported_modules.clone(),
            self.prefix_to_module.clone(),
        );
    ...
    fn process_imports_with_list<P: AsRef<Path>>(
        &mut self,
        base_path: P,
        current_module: &str,
            ...
            match yangfile {
                YangFile::Module(mut module) => {
                    ...
                    self.imported_modules
                        .insert(import.module.clone(), module_parser.get_reference_nodes().clone());

                    // Add any nested imports to our processing queue
                    for nested_import in module_parser.take_imports() {
    ...
    /// Get a map of imported modules with their reference nodes
    pub fn get_imported_modules(&self) -> &HashMap<String, ReferenceNodes> {
        &self.imported_modules
    }

    /// Get the mapping of prefix to module name
    pub fn get_prefix_to_module(&self) -> &HashMap<String, String> {
        &self.prefix_to_module
    }
}
